...
const CustomSelect: React.FC<{
  ...
  return (
    <Select onValueChange={onChange}>
      <SelectTrigger
        className={`w-[180px] bg-transparent text-white ${className} border-0 focus:ring-0 hover:bg-white/10`}
      >
        {withIcons && (
          <>
            {selected === "english" ? (
      ...
      <SelectContent className="bg-white cursor-pointer z-[9999999]">
        <SelectGroup>
          <SelectLabel></SelectLabel>
          {data.map(({ value, text }, idx: number) => (
            <SelectItem className="list-none" key={idx} value={value}>
