...
/** NOTE: removed all preview/fake data — this component now only renders backend data. */
...
export default function PropertyPreview(): JSX.Element {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const id = user?.host?.id ?? null;
        if (!id) {
          // no host id — clear properties to an empty array
          setPropertiesRaw([]);
          return;
        }
        ...
        setPropertiesRaw(propsRes);
        console.log("properties:", propsRes);
      } catch (err) {
        console.error("Failed to load properties:", err);
        setPropertiesRaw([]);
      } finally {
  ...
  const propertiesToRender = propertiesList;
    ...
    <div style={styles.container}>
      ...
      <div style={styles.headerRow}>
        ...
        <button
          ...
          aria-label="View all properties"
          title="View all properties"
          onClick={() => router.push("/hosts/dashboard/properties")}
        >
          View All Properties →
        </button>
      </div>
      ...
      <div style={gridStyle}>
        ...
        {!loading &&
          propertiesToRender.map((rawProp) => {
            ...
            const prop = rawProp;
            const pid = String(prop.id ?? prop._id ?? "");
            const title = prop.title ?? prop.name ?? "Untitled property";
            const type = prop.type ?? "—";
            const location = prop.location ?? "—";
            const status =
            ...
            return (
              <article
                key={pid || JSON.stringify(prop)} // fallback key if id missing
                style={{
                  ...styles.card,
                  ...(isHovered ? styles.cardHover : {}),
                  display: "flex",
                ...
                <div style={styles.details}>
                  ...
                  <div style={styles.actionWrap}>
                    <button
                      ...
                      onMouseLeave={() => setBtnHoverId(null)}
                      onClick={() =>
                        // navigate to the property detail page (normalize id)
                        router.push(`/hosts/dashboard/properties/${pid}`)
                      }
                      style={{
                        ...styles.viewBtn,
                        ...(isBtnHovered ? styles.viewBtnHover : {}),
