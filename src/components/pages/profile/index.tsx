...
import { getUserProfileById, updateUserById, verfiyNIN } from "@/http/api";
import { formatDate, formatUserProfileData } from "@/lib/utils";
...
export interface IndividualUserAccountDetails {
...
export default () => {
  ...
  const [state, setState] = useState<IndividualUserAccountDetails>(() => ({
    ...
    if (data.verificationStatus === "VERIFIED") {
      nameMatch =
        (data.response[0].firstname as string).toLowerCase() ===
          state.user.firstName.toLowerCase() &&
        (data.response[0].surname as string).toLowerCase() ===
          state.user.lastName.toLowerCase();
    } else {
  ...
  function updateProfile() {
    ...
    formData = formatUserProfileData(
      formData,
      {
        ...state,
        profile: {
