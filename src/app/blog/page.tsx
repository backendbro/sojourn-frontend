...
export default function BlogHome() {
  ...
  useEffect(() => {
    ...
    // Find featured post or use first one
    ...
    // Filter posts (non-featured)
    setFilteredPosts(posts.filter((post) => post.id !== featured?.id));
  }, []);

  // Filter posts based on category
  useEffect(() => {
    if (!allPosts.length || !featuredPost) return;

    let posts = allPosts.filter((post) => post.id !== featuredPost.id);
  ...
  }, [filter, allPosts, featuredPost]);

  const categories = ["all", "travel", "culture", "lifestyle", "food"];

  // Image slider for post cards
  const nextImage = (postId: string, category: string) => {
    setImageIndexes((prev) => ({
      ...prev,
      [postId]: (prev[postId] + 1) % 3, // We have 3 images per category
    }));
  };
                ...
                return (
                  <article key={post.id} className="blog-post-item">
                    <div
                      ...
                      onClick={() => nextImage(post.id, post.category)}
                    >
                      <div
