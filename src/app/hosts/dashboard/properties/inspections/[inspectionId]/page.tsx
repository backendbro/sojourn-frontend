...
import InspectionSchedule from "@/components/forms/inspection-schedule";
...
import { cancelInspectionById, getInspectionById } from "@/http/api";
...
type InspectionStatusKeys = keyof InspectionStatus;

const PropertyInspectionStatusList = ["pending", "approved", "cancelled"];
...
export default ({
  ...
  const inspectionComplete = data
    ? PropertyInspectionStatusList[Number(data?.status)] !== "pending"
    : null;

  function transformStatusKey(key: string) {
    if (key === "newProperty") {
      return "new property";
    }
    return key;
  ...
  useEffect(() => {
    if (data && data.inspectionDate) {
      const isCancelled =
        PropertyInspectionStatusList[Number(data.status)] === "cancelled";

      if (isCancelled) {
        setInspectionStatus((prevState) => ({
          request: false,
          inspection: false,
          decision: false,
          newProperty: false,
        ...
        const isPastInspectionDate = date <= new Date();
        const isDecisionMade =
          PropertyInspectionStatusList[Number(data.status)] !== "pending";
        const isPropertyApproved =
          PropertyInspectionStatusList[Number(data.status)] === "approved";
        setInspectionStatus((prevState) => ({
          ...prevState,
          inspection: isPastInspectionDate,
          decision: isDecisionMade,
          newProperty: isPropertyApproved,
        }));
      }
    }
  }, [data]);
  ...
  return (
    <div className="w-full flex flex-col min-h-[88vh] py-10 bg-white px-5 md:px-5 lg:px-20">
      <div className="w-full flex items-center ">
        <Link
          href="/hosts/dashboard/properties?tabState=inspections"
      ...
      <div className="w-full h-full flex flex-col">
        ...
        {data?.inspector ? (
          <div className="w-full my-5 space-y-2 flex flex-col justify-center">
            <h4 className="text-xl font-semibold underline">Inspector</h4>
            <div className="flex flex-col space-y-2">
              ...
              <p className="flex items-center space-x-2">
                <Mail size={15} />
                <span className="font-semibold">{data.inspector.email}</span>
              </p>
              <p className="flex items-center space-x-2">
                <Phone size={15} />
                <span className="font-semibold">
                  {data.inspector.phoneNumber}
                </span>
              </p>
            </div>
          </div>
        ) : (
          <div className="w-full my-10 space-y-2 flex flex-col justify-center">
            <h4 className="text-xl font-semibold underline">Inspector</h4>
        ...
        {editPropertyFormState.houseRules ? (
          ...
          <div className="w-full  pb-10 border-b-2 border-b-secondary mb-5">
            <div className="w-full min-h-[100px] grid md:grid-cols-3 lg:grid-cols-4 gap-3">
              <div className="col-span-1 md:col-span-3 lg:col-span-4 space-y-5 text-[14px[">
        ...
        <div className="w-full flex items-center justify-end">
          {editPropertyFormState.inspectionSchedule ? (
            <span
              onClick={() => {
                setPropertyFormState((prevState) => ({
                  ...prevState,
                ...
                <Tooltip>
                  <TooltipTrigger
                    onClick={(e) => {
                      setPropertyFormState((prevState) => ({
                        ...prevState,
                        inspectionSchedule: !prevState?.inspectionSchedule,
                      }));
                    }}
                  >
                    <Edit className="block" size={20} />
        ...
        {editPropertyFormState.inspectionSchedule ? (
          ...
          <div className="w-full flex flex-col">
            <h4 className="text-lg font-semibold mb-5">Inspection Schedule</h4>
            <div className="w-full flex items-center  space-x-3">
              <div className="flex items-center space-x-1">
                <span>
                  <Calendar size={15} />
                </span>
                <span>{new Date(data.inspectionDate).toDateString()}</span>
              </div>
              <div className="flex items-center space-x-1">
                <span>
                  <Clock size={15} />
                </span>
                <span>{data.inspectionTime}</span>
              </div>
            </div>
          </div>
        )}
          ...
          {editPropertyFormState.propertyAddress ? (
            ...
            <>
              ...
              <div className="w-full h-[400px]">
                <Map positions={[+data.lat, +data.lng]} />
              </div>
            </>
          )}
        </div>
        <div className="w-full py-10 flex flex-col space-y-10 border-b-2 border-b-secondary">
          <h3 className="font-semibold text-xl mb-10">Inspection Status</h3>
          <div className="w-full flex items-center mb-10">
            {Object.keys(inspectionStatus).map((k, idx) => {
              const css = inspectionStatus[k as InspectionStatusKeys]
                ? "bg-primary"
                : "bg-gray-200";

              const nextIdx = idx + 1;
              const nextProperty = Object.keys(inspectionStatus)[nextIdx];
              const barCss =
                inspectionStatus[k as InspectionStatusKeys] &&
                inspectionStatus[nextProperty as InspectionStatusKeys]
                  ? "bg-primary"
                  : "bg-gray-200";

              return (
                <div className="space-y-3 flex flex-col relative" key={idx}>
                  <span
                    className={`capitalize text-gray-600 absolute -top-7 ${
                      idx > 0 ? "-left-7" : "-left-1"
                    } ${idx > 0 ? "w-[150px]" : ""}`}
                  >
                    {transformStatusKey(k)}
                  </span>
                  <div className="flex items-center">
                    <span className={`w-4 h-4  rounded-full ${css}`}></span>
                    {idx !== Object.keys(inspectionStatus).length - 1 ? (
                      <div className={`w-20 md:w-44 h-1 ${barCss}`}></div>
                    ) : null}
                  </div>
                </div>
              );
          ...
          {PropertyInspectionStatusList[Number(data?.status)] ===
          "cancelled" ? (
            <div className="mb-5">
              This inspection was cancelled, kindly contact support for further
              details.
            </div>
          ) : null}
          {!inspectionComplete ? (
            <div className="flex items-center">
              <button
                onClick={() => {
                  dispatch(
                    setPromptQuestion(
                      "Are you sure you want cancel this inspection ?"
                    )
                  );
                  dispatch(setId(data.id));
                  dispatch(setPage("inspection"));
                  dispatch(openPrompt("open"));
                }}
                className="flex items-center space-x-2 py-3 rounded-full px-14 border border-primary hover:bg-red-50"
              >
